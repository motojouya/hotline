...
window.addEventListner('DOMContentLoaded', function (event) {
  ...
  var checkUserid = function checkUserid() {
    _superagent2.default.get('/api/v1/checkuserid').query({ userid: useridEle.value }).end(function (err, res) {
      if (err) {
  ...
  registerExeBtn.addEventListner('onclick', function (event) {
    ...
    _superagent2.default.post('/api/v1/register').send({
      userid: userid,
      email: email,
